...
function App() {
  ...
  const rollDice = () => {
    ...
    const newIsWinner = newDiceValues.every(
      (value) => value === newDiceValues[0]
    );
      ...
      <div className="grid grid-cols-3 gap-4 mb-8">
        {diceValues.map((value, index) => {
          ...
          return (
            <div
              key={index}
              className="bg-white p-4 rounded-lg shadow-md flex items-center justify-center transform hover:scale-110 transition-transform duration-200"
            >
          ...
          <p className="text-2xl font-semibold mb-2">
            {isWinner ? (
              <span className="text-green-600">
                Congratulations! You won in {rollCount} rolls!
              </span>
            ) : (
              <span className="text-red-600">
                Try again! (Roll count: {rollCount})
              </span>
            )}
          </p>
          <p className="text-xl">
            Best score: {bestScore === Infinity ? "N/A" : bestScore} rolls
          </p>
          {isWinner && (
            <button
              onClick={resetGame}
